...
import LintResultCache from "./lint-result-cache";
import loadRules from "./load-rules";

const { ConfigOps, naming, IgnorePattern, getUsedExtractedConfigs, ModuleResolver, CascadingConfigArrayFactory } = Legacy;
...
export interface CLIEngineOptions {
    ...
    baseConfig?: ConfigData | false | null;
    cache?: boolean;
...
export interface CLIEngineInternalSlots {
    additionalPluginPool: Map<string, Plugin>;
    cacheFilePath: string;
    configArrayFactory: InstanceType<typeof CascadingConfigArrayFactory>;
    defaultIgnores: (filePath: string) => boolean;
    fileEnumerator: FileEnumerator;
    lastConfigArrays: ConfigArray[];
    lintResultCache: LintResultCache | null;
    linter: InstanceType<typeof Linter>;
    options: ResolvedCLIEngineOptions;
}
...
const internalSlotsMap = new WeakMap<CLIEngine, CLIEngineInternalSlots>();
 ...
 */
function validateFixTypes(fixTypes: string[]) {
...
function calculateStatsPerFile(messages: LintMessage[]) {
    return messages.reduce<Stat>(
        (stat, message) => {
 ...
 * @private
 */
function calculateStatsPerRun(results: LintResult[]): Stat {
    return results.reduce<Stat>(
        (stat, result) => {
...
function verifyText(options: VerifyTextOptions) {
    const {
        text,
        cwd,
    ...
    const { fixed, messages, output } = linter.verifyAndFix(text, config, {
         ...
         */
        filterCodeBlock(blockFilename: string) {
 ...
 * @private
 */
function createIgnoreResult(filePath: string, baseDir: string): LintResult {
    let message;
 ...
 */
function getRule(ruleId: string, configArrays: any[]) {
 ...
 */
function shouldMessageBeFixed(message: LintMessage, lastConfigArrays: any[], fixTypes: Set<string>) {
    if (!message.ruleId) {
 ...
 */
function* iterateRuleDeprecationWarnings(usedConfigArrays: any[]) {
 ...
 * @private
 */
function isErrorMessage(message: LintMessage) {
 ...
 */
function getCacheFile(cacheFile: string, cwd: string) {
    /*
 ...
 */
function toBooleanMap(keys: string[] | undefined, defaultValue: boolean, displayName: string) {
    if (keys && !Array.isArray(keys)) {
        throw new Error(`${displayName} must be an array.`);
    }
    if (keys && keys.length > 0) {
        return keys.reduce<Record<string, boolean>>((map, def) => {
 ...
 */
function createConfigDataFromOptions(options: CLIEngineOptions) {
 ...
 */
function directoryExists(resolvedPath: string): boolean {
    try {
...
export type HasDefaultValueFields =
    | "cwd"
    | "configFile"
    | "baseConfig"
    | "rulePaths"
...
export interface ResolvedCLIEngineOptions
    extends Omit<CLIEngineOptions, HasDefaultValueFields>,
        Required<Pick<CLIEngineOptions, HasDefaultValueFields>> {
    cacheFile: string;
    cacheStrategy: "metadata" | "content";
}
...
class CLIEngine {
    ...
    static getFormatter: (format: string) => FormatterFunction | null;
     ...
     */
    constructor(providedOptions: CLIEngineOptions, { preloadedPlugins }: { preloadedPlugins?: Record<string, Plugin> | null } = {}) {
        const options: ResolvedCLIEngineOptions = Object.assign(
            Object.create(null),
            defaultOptions,
            { cwd: process.cwd() },
            providedOptions
        ...
        if (options.fix && options.fixTypes && options.fixTypes.length > 0) {
            ...
            const fixTypes: Set<string> = new Set(options.fixTypes);

            // save original value of options.fix in case it's a function
            const originalFix = typeof options.fix === "function" ? options.fix : () => true;

            // @ts-expect-error
            options.fix = (message: LintMessage) =>
                // @ts-expect-error
                shouldMessageBeFixed(message, lastConfigArrays, fixTypes) && originalFix(message);
        }
    }
     ...
     * @returns {void}
     */
    static outputFixes(report: LintReport) {
        report.results
            .filter(result => Object.prototype.hasOwnProperty.call(result, "output"))
            .forEach(result => {
     ...
     */
    resolveFileGlobPatterns(patterns: string[]) {
        const slots = internalSlotsMap.get(this);
        assert(!!slots);
        ...
        const extensions = (options.extensions || [".js"]).map((ext: string) => ext.replace(/^\./u, ""));
    ...
    executeOnFiles(patterns: string[]): LintReport {
        const currentEngine = internalSlotsMap.get(this);
        assert(!!currentEngine);
     ...
     */
    executeOnText(text: string, filename: string, warnIgnored: boolean): LintReport {
        const currentEngine = internalSlotsMap.get(this);
        if (!currentEngine) {
            throw new Error();
        }
    ...
    getConfigForFile(filePath: string) {
        ...
        const { configArrayFactory, options } = currentEngine;
 ...
 */
function getCLIEngineInternalSlots(instance: CLIEngine) {
    const slot = internalSlotsMap.get(instance);
    if (!slot) {
        throw new Error();
    }
    return slot;
...
export { CLIEngine, getCLIEngineInternalSlots };
