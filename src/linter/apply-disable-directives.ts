 ...
 */
function compareLocations(itemA: { line: number; column: number }, itemB: { line: number; column: number }) {
 ...
 */
function groupByParentComment(directives: Directive[]): Directive[][] {
    const groups: Map<string, Directive[]> = new Map();
 ...
 */
function createIndividualDirectivesRemoval(directives: Directive[], commentToken: Token) {
    /*
    ...
    // FIXME
    const listStartOffset = /^\s*\S+\s+/u.exec(commentToken.value)?.[0].length ?? 0;
 ...
 */
function createCommentRemoval(directives: Directive[], commentToken: Token) {
 ...
 * @returns {{ description, fix, unprocessedDirective }[]} Details for later creation of output Problems.
 */
function processUnusedDisableDirectives(allDirectives: Directive[]) {
...
function applyDirectives(options: Options) {
    ...
    const unusedDisableDirectivesToReport = options.directives.filter(
        directive => directive.type === "disable" && !usedDisableDirectives.has(directive)
    );
...
export = (options: Options) => {
    const { directives, disableFixes, problems, reportUnusedDisableDirectives = "off" } = options;
