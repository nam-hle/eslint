...
import { isPosition, MessageDescriptor } from "../shared/types";
import SourceCode from "../source-code/source-code";

import interpolate from "./interpolate";
import ruleFixer from "./rule-fixer";
 ...
 */
function normalizeMultiArgReportCall(...args: any[]) {
 ...
 */
function assertValidNodeInfo(descriptor: MessageDescriptor) {
    if (descriptor.node) {
 ...
 */
function normalizeReportLoc(descriptor: MessageDescriptor): {
    start: Position;
    end: Position | null;
} {
    if (descriptor.loc) {
        if (!isPosition(descriptor.loc)) {
 ...
 * @returns {void}
 */
function assertValidFix(fix: Fix | null) {
    if (fix) {
        assert(
 ...
 * @private
 */
function compareFixesByRange(a: Fix, b: Fix) {
 ...
 */
function mergeFixes(fixes: Fix[], sourceCode: SourceCode) {
 ...
 */
function mapSuggestions(descriptor: MessageDescriptor, sourceCode: SourceCode, messages: any) {
 ...
 * @returns {void}
 */
function validateSuggestions(suggest: { desc?: string; messageId?: string; fix?: Fix }[], messages: any) {
        ...
        suggest.forEach(suggestion => {
            if (suggestion.messageId) {
                ...
                if (!messages[messageId]) {
                    throw new TypeError(
                        `context.report() called with a suggest option with a messageId '${messageId}' which is not present in the 'messages' config: ${JSON.stringify(
                            messages,
                            null,
                            2
                        )}`
...
export = function createReportTranslator(metadata: {
    ruleId: string;
    severity: 0 | 1 | 2;
