...
import { parseRuleId, getRuleFromConfig, getRuleOptionsSchema } from "./flat-config-helpers";
 ...
 * @returns {void}
 */
function throwRuleNotFoundError({ pluginName, ruleName }: { pluginName: string; ruleName: string }, config: Record<string, any>) {
 ...
 */
class RuleValidator {
    private validators: WeakMap<object, any>;
    /**
        ...
        for (const [ruleId, ruleOptions] of Object.entries(config.rules)) {
            ...
            if (validateRule) {
                ...
                if (validateRule.errors) {
                    throw new Error(
                        `Key "rules": Key "${ruleId}": ${validateRule.errors
                            .map((error: ErrorObject) => `\tValue ${JSON.stringify(error.data)} ${error.message}.\n`)
                            .join("")}`
                    );
                }
            }
