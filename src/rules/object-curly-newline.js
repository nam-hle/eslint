...
function areLineBreaksRequired(node, options, first, last) {
    ...
    return (
        ...
        (options.multiline && objectProperties.length > 0 && first.loc.start.line !== last.loc.end.line)
    );
}
...
module.exports = {
    ...
    create(context) {
        ...
        function check(node) {
            ...
            if (
                (node.type === "ImportDeclaration" && !node.specifiers.some(specifier => specifier.type === "ImportSpecifier")) ||
                (node.type === "ExportNamedDeclaration" && !node.specifiers.some(specifier => specifier.type === "ExportSpecifier"))
            ) {
                return;
            }
