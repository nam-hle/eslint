...
import { LintReport } from "../cli-engine/cli-engine";
import LintResultCache from "../cli-engine/lint-result-cache";
...
const debug = Debug("eslint:flat-eslint");
 ...
 * @private
 */
function calculateStatsPerRun(results: LintResult[]) {
 ...
 */
function createRulesMeta(rules: Map<string, Rule>) {
    return Array.from(rules).reduce<Record<string, JSONSchema4>>((retVal, [id, rule]) => {
        // @ts-expect-error
        retVal[id] = rule.meta;
        return retVal;
    }, {});
 ...
 */
function getPlaceholderPath(cwd: string) {
    return path.join(cwd, "__placeholder__.js");
}

/** @type {WeakMap<ExtractedConfig, DeprecatedRuleInfo[]>} */
const usedDeprecatedRulesCache = new WeakMap<ExtractedConfig, DeprecatedRuleInfo[]>();
 ...
 */
function getOrFindUsedDeprecatedRules(eslint: CLIEngine, maybeFilePath: string): DeprecatedRuleInfo[] {
    const {
        configs,
        options: { cwd }
 ...
 */
function processLintReport(eslint: CLIEngine, { results }: LintReport): LintResult[] {
    const descriptor = {
        configurable: true,
        enumerable: true,
        // @ts-expect-error
 ...
 */
function compareResultsByFilePath(a: LintResult, b: LintResult) {
 ...
 */
function findFlatConfigFile(cwd: string): Promise<string | undefined> {
 ...
 */
async function loadFlatConfigFile(filePath: string) {
     ...
     */
    fileURL.searchParams.append("mtime", String(mtime));
...
function verifyText(config: {
    ...
    const { text, cwd, filePath: providedFilePath, configs, fix, allowInlineConfig, reportUnusedDisableDirectives, linter } = config;
 ...
 */
function shouldMessageBeFixed(message: LintMessage, config: any, fixTypes: Set<string>) {
    if (!message.ruleId) {
 ...
 */
function* iterateRuleDeprecationWarnings(configs: any[]) {
...
class FlatESLint {
    ...
    constructor(options: FlatESLintOptions = {}) {
        const defaultConfigs: { plugins: Record<string, Plugin> }[] = [];
     ...
     */
    static async outputFixes(results: LintResult[]) {
     ...
     */
    getRulesMetaForResults(results: LintResult[]) {
    ...
    async lintFiles(patterns: string | string[]) {
        ...
        const { cacheFilePath, lintResultCache, linter, options: eslintOptions } = privateMembers.get(this);
        const configs = await calculateConfigArray(this, eslintOptions);
        const { allowInlineConfig, cache, cwd, fix, fixTypes, reportUnusedDisableDirectives, globInputPaths, errorOnUnmatchedPattern } =
            eslintOptions;
        const startTime = Date.now();
        const usedConfigs: any[] = [];
        const fixTypesSet = fixTypes ? new Set<string>(fixTypes) : null;
        ...
        const results = await Promise.all(
            ...
            filePaths.map(({ filePath, ignored }) => {
                ...
                if (fix && fixTypesSet) {
                    ...
                    fixer = (message: LintMessage) => shouldMessageBeFixed(message, config, fixTypesSet) && originalFix(message);
                }
     ...
     */
    async lintText(code: string, options: { filePath?: string; warnIgnored?: boolean } = {}) {
        ...
        const configs = await calculateConfigArray(this, eslintOptions);
        const { allowInlineConfig, cwd, fix, reportUnusedDisableDirectives } = eslintOptions;
        const results = [];
        const startTime = Date.now();
    ...
    async loadFormatter(name = "stylish") {
        ...
        return {
             ...
             */
            format(results: LintResult[], resultsMeta: ResultsMeta) {
                let rulesMeta: any = null;
     ...
     */
    async calculateConfigForFile(filePath: string): Promise<ConfigData | undefined> {
     ...
     */
    async isPathIgnored(filePath: string) {
...
export { FlatESLint, findFlatConfigFile };
