 ...
 */
function isArrayOfNonEmptyString(x: any) {
 ...
 */
function processOptions(options: CLIEngineOptions): CLIEngineOptions {
    const {
 ...
 */
function hasDefinedProperty(obj: any) {
...
function createRulesMeta(rules: Map<string, Rule>) {
    return Array.from(rules).reduce<Record<string, RuleMeta>>((retVal, [id, rule]) => {
        // @ts-expect-error
        retVal[id] = rule.meta;
        return retVal;
    }, {});
 ...
 */
function getOrFindUsedDeprecatedRules(cliEngine: CLIEngine, maybeFilePath: string) {
    const {
        configArrayFactory,
        options: { cwd }
...
class ESLint {
    ...
    constructor(options?: CLIEngineOptions = {}) {
        ...
        const { configArrayFactory, lastConfigArrays } = getCLIEngineInternalSlots(cliEngine);
        let updated = false;
     ...
     */
    async lintText(code: string, options: any = {}) {
    ...
    async loadFormatter(name = "stylish") {
        ...
        return {
            ...
            format(results: LintResult[], resultsMeta: ResultsMeta) {
                let rulesMeta: ResultsMeta | null = null;
     ...
     */
    async calculateConfigForFile(filePath: string) {
 ...
 */
function getESLintPrivateMembers(instance: ESLint): ESLintPrivateMembers {
    return privateMembersMap.get(instance);
}

export { ESLint, getESLintPrivateMembers };
