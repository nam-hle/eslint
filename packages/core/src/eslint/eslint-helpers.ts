...
class NoFilesFoundError extends Error {
     ...
     */
    constructor(pattern: string, globEnabled: boolean) {
 ...
 */
function normalizeToPosix(pattern: string) {
 ...
 */
function globMatch({ basePath, pattern }: { basePath: string; pattern: string }) {
    let found = false;
...
async function globMultiSearch({
    ...
    searches: Map<string, GlobSearch>;
    configs: any;
    errorOnUnmatchedPattern: boolean;
}) {
    /*
 ...
 */
function fileExists(resolvedPath: string) {
    try {
 ...
 * @private
 */
function createIgnoreResult(filePath: string, baseDir: string) {
    let message;
 ...
 */
function processOptions(options: FlatESLintOptions) {
    const {
