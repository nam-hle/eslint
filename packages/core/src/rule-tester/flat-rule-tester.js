...
class FlatRuleTester {
    ...
    constructor(testerConfig = {}) {
         ...
         * @type {Object}
         */
        this.testerConfig = [sharedDefaultConfig, testerConfig, { rules: { "rule-tester/validate-ast": "error" } }];
    ...
    run(ruleName, rule, test) {
        ...
        function testValidTemplate(item) {
            ...
            assert.strictEqual(
                messages.length,
                0,
                util.format("Should have no errors but had %d: %s", messages.length, util.inspect(messages))
            );
                        ...
                        if (hasOwnProperty(error, "suggestions")) {
                            ...
                            if (!error.suggestions || (Array.isArray(error.suggestions) && error.suggestions.length === 0)) {
                                ...
                                assert.strictEqual(
                                    ...
                                    true,
                                    `Error should have an array of suggestions. Instead received "${message.suggestions}" on error with message: "${message.message}"`
                                );
                                assert.strictEqual(
