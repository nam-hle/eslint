...
import { Token, ASTNode } from "@eslint/types";
...
class FixTracker {
     ...
     */
    retainRange(range: [number, number] | undefined) {
        this.retainedRange = range ?? null;
        return this;
    }
     ...
     * @returns {FixTracker} The same RuleFixer, for chained calls.
     */
    retainSurroundingTokens(nodeOrToken: ASTNode | Token) {
     ...
     */
    replaceTextRange(range: number[], text: string) {
        let actualRange;
