 ...
 */
function isES5Constructor(node: ASTNode) {
    // @ts-expect-error
 ...
 */
function getUpperFunction(node: ASTNode) {
 ...
 */
function isLoop(node: ASTNode | null) {
 ...
 */
function isInLoop(node: ASTNode | null) {
    for (let currentNode: ASTNode | undefined | null = node; currentNode && !isFunction(currentNode); currentNode = currentNode.parent) {
 ...
 */
function isNullLiteral(node: ASTNode) {
    /*
 ...
 * @public
 */
function isNullOrUndefined(node: ASTNode) {
    return (
        isNullLiteral(node) ||
 ...
 */
function isCallee(node: ASTNode) {
 ...
 */
function getStaticStringValue(node: ASTNode) {
    switch (node.type) {
        case "Literal":
            if (node.value === null) {
 ...
 */
function getStaticPropertyName(node: ASTNode): string | null {
    ...
    if (prop) {
        ...
        return getStaticStringValue(prop) ?? null;
    }
 ...
 */
function skipChainExpression(node: ASTNode) {
 ...
 */
function isSpecificId(node: ASTNode, name: string | RegExp) {
 ...
 */
function isSpecificMemberAccess(node: ASTNode, objectName: string | RegExp | null, propertyName: string | RegExp | null) {
 ...
 */
function equalLiteralValue(left: ASTNode, right: ASTNode) {
...
/**
 * Check if two expressions reference the same value. For examplakDe:
 *     a = a
 *     a.b = a.b
 *     a[0] = a[0]
 *     a['b'] = a['b']
 ...
 */
function isSameReference(left: ASTNode, right: ASTNode, disableStaticComputedKey = false): boolean {
        ...
        case "MemberExpression": {
            if (!disableStaticComputedKey) {
                ...
                if (nameA !== null) {
                    return (
                        // @ts-expect-error
                        isSameReference(left.object, right.object, disableStaticComputedKey) && nameA === getStaticPropertyName(right)
                    );
                }
            }
 ...
 */
function isReflectApply(node: ASTNode) {
 ...
 */
function isArrayFromMethod(node: ASTNode) {
 ...
 */
function isMethodWhichHasThisArg(node: ASTNode) {
 ...
 */
function negate(f: (...args: any) => any) {
    return (token: any) => !f(token);
}
 ...
 */
function hasJSDocThisTag(node: ASTNode, sourceCode: SourceCode) {
 ...
 */
function isLogicalExpression(node: ASTNode) {
 ...
 */
function isCoalesceExpression(node: ASTNode) {
 ...
 */
function isMixedLogicalAndCoalesceExpressions(left: ASTNode, right: ASTNode) {
 ...
 */
function isLogicalIdentity(node: ASTNode, operator: string): boolean {
    switch (node.type) {
        case "Literal":
 ...
 */
function isReferenceToGlobalVariable(scope: any, node: ASTNode) {
    const reference = scope.references.find((ref: any) => ref.identifier === node);
 ...
 * @private
 */
function isConstant(scope: any, node: ASTNode, inBooleanPosition: boolean): boolean {
...
export = {
     ...
     * @public
     */
    isTokenOnSameLine(left: any, right: any) {
     ...
     */
    isBreakableStatement(node: ASTNode) {
     ...
     */
    getVariableByName(initScope: any, name: string) {
        let scope = initScope;
     ...
     */
    isDefaultThisBinding(node: ASTNode, sourceCode: SourceCode, { capIsConstructor = true } = {}) {
        /*
            ...
            switch (parent.type) {
                ...
                case "CallExpression":
                    if (isReflectApply(parent.callee)) {
                        return (
                            parent.arguments.length !== 3 || parent.arguments[0] !== currentNode || isNullOrUndefined(parent.arguments[1])
                        );
                    }
                    if (isArrayFromMethod(parent.callee)) {
                        return (
                            parent.arguments.length !== 3 || parent.arguments[1] !== currentNode || isNullOrUndefined(parent.arguments[2])
                        );
                    }
                    if (isMethodWhichHasThisArg(parent.callee)) {
                        return (
                            parent.arguments.length !== 2 || parent.arguments[0] !== currentNode || isNullOrUndefined(parent.arguments[1])
                        );
                    }
                    return true;
     ...
     */
    isEmptyBlock(node: ASTNode | null) {
     ...
     */
    getDirectivePrologue(node: ASTNode) {
        const directives = [];
     ...
     */
    getFunctionNameWithKind(node: ASTNode) {
     ...
     */
    getFunctionHeadLoc(node: ASTNode, sourceCode: SourceCode) {
        const parent = node.parent;
        let start = null;
        let end = null;
     ...
     */
    getNextLocation(sourceCode: SourceCode, { line, column }: { line: number; column: number }): { line: number; column: number } | null {
     ...
     */
    canTokensBeAdjacent(leftValue: Token | string, rightValue: Token | string) {
        const espreeOptions = {
     ...
     */
    getNameLocationInGlobalDirectiveComment(sourceCode: SourceCode, comment: Comment, name: string) {
