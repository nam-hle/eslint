...
module.exports = {
    ...
    create(context) {
        ...
        function startFunction(node) {
            fns.push({
                returnPresent:
                    (node.type === "ArrowFunctionExpression" && node.body.type !== "BlockStatement") || isTypeClass(node) || node.async
            });
        }
                ...
                jsdoc.tags.forEach(tag => {
                    ...
                    if (Object.prototype.hasOwnProperty.call(prefer, tag.title) && tag.title !== prefer[tag.title]) {
                        ...
                        context.report({
                            ...
                            fix(fixer) {
                                return fixer.replaceTextRange(
                                    [jsdocNode.range[0] + tag.range[0] + 3, jsdocNode.range[0] + tag.range[0] + tag.title.length + 3],
                                    prefer[tag.title]
                                );
                            }
                        });
