...
function endsWithTemplateCurly(node) {
    ...
    if (node.type === "TemplateLiteral") {
        return (
            ...
            node.quasis.length &&
            node.quasis[node.quasis.length - 1].range[0] === node.quasis[node.quasis.length - 1].range[1]
        );
    }
    return node.type !== "Literal" || typeof node.value !== "string";
}
...
module.exports = {
    ...
    create(context) {
        ...
        function getTextBetween(node1, node2) {
            ...
            return allTokens
                .slice(0, -1)
                .reduce((accumulator, token, index) => accumulator + sourceText.slice(token.range[1], allTokens[index + 1].range[0]), "");
        }
        ...
        function getTemplateLiteral(currentNode, textBeforeNode, textAfterNode) {
            ...
            if (isConcatenation(currentNode) && hasStringLiteral(currentNode)) {
                ...
                if (leftEndsWithCurly) {
                    ...
                    return (
                        getTemplateLiteral(currentNode.left, textBeforeNode, textBeforePlus + textAfterPlus).slice(0, -1) +
                ...
                if (rightStartsWithCurly) {
                    ...
                    return (
                        getTemplateLiteral(currentNode.left, textBeforeNode, null).slice(0, -1) +
                 ...
                 */
                return `${getTemplateLiteral(currentNode.left, textBeforeNode, null)}${textBeforePlus}+${textAfterPlus}${getTemplateLiteral(
                    currentNode.right,
                    textAfterNode,
                    null
                )}`;
