...
const { isCommaToken, isOpeningParenToken, isClosingParenToken, isParenthesised } = require("./utils/ast-utils");
            ...
            Program() {
                ...
                for (const { node } of tracker.iterateGlobalReferences(trackMap)) {
                    if (
                        ...
                        !hasArraySpread(node) &&
                        !(node.arguments.length > 1 && hasArgumentsWithAccessors(node))
                    ) {
