...
module.exports = {
    ...
    create(context) {
        ...
        const IGNORE_FUNCTION_PROTOTYPE_METHODS =
            ALL_NODES && context.options[1] && context.options[1].enforceForFunctionPrototypeMethods === false;
        ...
        return {
                ...
                if (node.init) {
                    if (node.init.type !== "VariableDeclaration") {
                        ...
                        if (
                            ...
                            astUtils.isOpeningBracketToken(sourceCode.getTokenAfter(firstToken, astUtils.isNotClosingParenToken))
                        ) {
            ...
            YieldExpression(node) {
                if (node.argument) {
                    ...
                    if (
                        (precedence(node.argument) >= precedence(node) && hasExcessParensNoLineTerminator(yieldToken, node.argument)) ||
