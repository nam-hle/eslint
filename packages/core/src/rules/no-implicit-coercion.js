...
function isMultiplyByOne(node) {
    return (
        node.operator === "*" &&
        ((node.left.type === "Literal" && node.left.value === 1) || (node.right.type === "Literal" && node.right.value === 1))
    );
}
...
function isStringType(node) {
    return (
        ...
        (node.type === "CallExpression" && node.callee.type === "Identifier" && node.callee.name === "String")
    );
}
...
function isEmptyString(node) {
    return (
        astUtils.isStringLiteral(node) &&
        (node.value === "" || (node.type === "TemplateLiteral" && node.quasis.length === 1 && node.quasis[0].value.cooked === ""))
    );
}
...
function isConcatWithEmptyString(node) {
    return (
        node.operator === "+" &&
        ((isEmptyString(node.left) && !isStringType(node.right)) || (isEmptyString(node.right) && !isStringType(node.left)))
    );
}
