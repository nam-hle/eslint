...
const debug = Debug("eslint:code-path");
 ...
 */
/* c8 ignore next */
function getId(segment: CodePathSegment) {
 ...
 */
function nodeToString(node: ASTNode, label: "enter" | "exit" | undefined) {
...
export = {
    ...
    dumpState: !debug.enabled
        ? debug
        : /* c8 ignore next */ function (node: ASTNode, state: CodePathState, leaving: boolean) {
    ...
    dumpDot: !debug.enabled
        ? debug
        : /* c8 ignore next */ function (codePath: CodePath) {
              let text =
                  "\n" +
                  "digraph {\n" +
     ...
     */
    makeDotArrows(codePath: CodePath, traceMap: any) {
        ...
        codePath.returnedSegments.forEach((finalSegment: CodePathSegment) => {
        ...
        codePath.thrownSegments.forEach((finalSegment: CodePathSegment) => {
            if (lastId === finalSegment.id) {
                text += "->thrown";
            } else {
