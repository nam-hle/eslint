 ...
 */
function renderSummary(totalErrors: number, totalWarnings: number) {
 ...
 */
function messageTemplate(it: any) {
    const { parentIndex, lineNumber, columnNumber, severityNumber, severityName, message, ruleUrl, ruleId } = it;
 ...
 */
function renderMessages(messages: FormatterMessage[], parentIndex: number, rulesMeta: any) {
    /**
 ...
 */
function resultTemplate(it: any) {
 ...
 */
function renderResults(results: any[], rulesMeta: any) {
    return results
        .map(
            (result, index) =>
                resultTemplate({
...
export = function (results: { errorCount: number; warningCount: number }[], data: any) {
    let totalErrors = 0,
        totalWarnings = 0;
