 ...
 */
export interface ConfigArrayElement {
    name: string;
    filePath: string;
 ...
 * @returns {void}
 */
function mergeWithoutOverwrite(target: Record<string, any>, source: Record<string, any> | undefined) {
 ...
 * @returns {void}
 */
function mergePlugins(target: Record<string, ResolvedPlugin>, source: Record<string, ResolvedPlugin>) {
    if (!isNonNullObject(source)) {
        return;
    }
 ...
 * @returns {void}
 */
function mergeRuleConfigs(target: Record<string, any[]>, source: Record<string, RuleConf> | undefined) {
    if (!isNonNullObject(source)) {
        return;
    }
...
function createConfig(instance: ConfigArray, indices: number[]): ExtractedConfig {
    ...
    for (const index of indices) {
        ...
        if (config.noInlineConfig === undefined && element.noInlineConfig !== undefined) {
            config.noInlineConfig = element.noInlineConfig;
            config.configNameOfNoInlineConfig = element.name;
        }

        // Adopt the reportUnusedDisableDirectives which was found at first.
        if (config.reportUnusedDisableDirectives === undefined && element.reportUnusedDisableDirectives !== undefined) {
 ...
 * @returns {void}
 */
function deleteMutationMethods(map: Map<unknown, unknown>) {
    Object.defineProperties(map, {
        clear: { configurable: true, value: undefined },
        delete: { configurable: true, value: undefined },
        set: { configurable: true, value: undefined }
    });
}
